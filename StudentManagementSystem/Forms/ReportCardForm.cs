...
namespace StudentManagementSystem.Forms
{
    public partial class ReportCardForm : Form
    {
        ...
        private void RenderTextReport()
        {
            ...
            sb.AppendLine("NSBM GREEN UNIVERSITY");
            ...
            sb.AppendLine();
            sb.AppendLine("END OF REPORT CARD");
        ...
        {
            if (_selectedStudent == null || string.IsNullOrWhiteSpace(rtbReportPreview.Text) ||
                rtbReportPreview.Text.StartsWith("Please enter") ||
                rtbReportPreview.Text.StartsWith("No student found"))
            {
                MessageBox.Show("Please search and select a student first before exporting.", "No Student Selected",
            ...
            {
                sfd.Title = "Export Student Report Card";
                sfd.Filter = "Text File (*.txt)|*.txt|CSV File (*.csv)|*.csv|All Files (*.*)|*.*";
                sfd.FileName = $"ReportCard_{_selectedStudent.RegNumber}_{DateTime.Now:yyyyMMdd}";
                sfd.DefaultExt = "txt";
                ...
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        MessageBox.Show($"Report card successfully exported to:\n\n{sfd.FileName}",
                            "Export Successful", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"Failed to export report card:\n{ex.Message}", "Export Error",
